...
export const PostNavigationItem: FC<Props> = ({ id, title, type }) => {
    return (
        <Link
            ...
            className={cn(
                'flex items-center gap-4 p-4 rounded-lg transition-all duration-300 bg-transparent hover:bg-gray-700/30',
                type === 'next' ? 'text-right' : 'text-left'
            )}>
            {type === 'prev' && <CircleChevronLeft className='text-gray-400 h-6 w-6' />}
            <div className='flex flex-col'>
                <h5 className='text-sm text-gray-500/90'>{type === 'next' ? '다음' : '이전'} 포스트</h5>
